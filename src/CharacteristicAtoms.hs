...
module CharacteristicAtoms where
...
instance (Eq a, Ord a, Show a) => Embed a (ChAtom a) where
  ...
  embed at1@(ChAtom a1 ch1) at2@(ChAtom a2 ch2) =
    homeo at1 at2 && not (CPD.isStrictInst at1 at2)
...
divingTree (ChRoot ch1) (ChRoot ch2)
  | length ch1 < length ch2 = any (all (uncurry homeoTree) . zip ch1) $ CPD.subconjs ch2 (length ch1)
