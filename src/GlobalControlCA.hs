...
isBigger :: GlobalTreeCh -> GC.GlobalTree -> Bool
isBigger (NodeCh _ _ _ ts1) (GC.Node _ _ _ ts2)
  = any (uncurry isBigger) (zip ts1 ts2)
isBigger (NodeCh _ _ _ _) (GC.Leaf _ _ _) = True
isBigger _ _ = False

isLess :: GlobalTreeCh -> GC.GlobalTree -> Bool
isLess (NodeCh _ _ _ ts1) (GC.Node _ _ _ ts2)
  = any (uncurry isLess) (zip ts1 ts2)
isLess (LeafCh _ _ _) (GC.Node _ _ _ _) = True
isLess _ _ = False

globalChToGlobal :: GlobalTreeCh -> GC.GlobalTree
globalChToGlobal (LeafCh (CPD.Descend chatom ancs) gen sig)
  = GC.Leaf (CPD.Descend (CA.chAtom chatom) (Set.map CA.chAtom ancs)) gen sig
globalChToGlobal (NodeCh (CPD.Descend chatom ancs) gen sld gts)
  = GC.Node (CPD.Descend (CA.chAtom chatom) (Set.map CA.chAtom ancs)) gen (CA.innerToSld sld)
    (globalChToGlobal <$> gts)
...
generalize :: CA.Atom S -> CA.Atom S -> E.Delta -> ([(CA.Atom S, T.Generalizer)], E.Delta)
generalize m b d =
  let ((m1, m2), delta) = CPD.split d b m
      (generalized, _, gen, delta') = D.generalizeGoals d m1 b
  in  (fmap (,gen) $ CPD.mcs generalized ++ CPD.mcs m2, delta')
...
whistleCA ancs m = let res = find (\b -> CPD.embed b m && not (CPD.isVariant b m)) ancs
  in trace' ("\n\nWHISTLE:\n" ++ show (Set.filter (\b -> CPD.embed b m && not (CPD.isVariant b m)) ancs))
  res

-- Do we really need it?
type Abstracted = ([(CA.ChAtom S, T.Generalizer)], E.Delta)

data AbstractedChild = AbstractedChild {
  absSubst :: E.Sigma
  , absAtom :: CA.ChAtom S
  , absGen  :: T.Generalizer
  , absGamma :: E.Gamma
}
...
generalizeCA env subst m b d =
  ...
  in trace' ("\n!!!!!!\nGeneralize: " ++ show atomB ++ " " ++ show atomM ++ "\n") $ let
      ((m1, m2), delta) = CPD.split d atomB atomM
      (generalized, _, gen, delta') = D.generalizeGoals d m1 atomB
      res = CPD.mcs generalized ++ CPD.mcs m2
--  in trace' ("\nRes: " ++ show res ++  "\n??????\n") $ let

      ch1 = CA.atomToChAtom env subst m1
      genchtree = CA.msgChTrees [CA.chTree b, CA.chTree ch1]
 -- in trace ("\n\nM: " ++ show m ++ "\nM2: " ++ show m2 ++ "\nB: " ++ show b ++ "\nM1: " ++ show ch1 ++
 --           "\nRes: " ++ show res ++ " " ++  show genchtree ++ "\n\n") $ let
      --(generalized, _, gen, delta') = D.generalizeGoals d atomB atomM
      --res = [generalized]      genchtree = CA.msgChTrees [CA.chTree b, CA.chTree m]
...
{-
abstractChild'' :: [CA.ChAtom S] -> Ancestors -> [CA.Resultant] -> [AbstractedChild]
abstractChild'' seen ancs bodies = let
    chatoms = abstractChildIn (Set.toList ancs) seen [] (CA.resultantToChAtom <$> bodies)
    in undefined
...
--abstractChildIn :: [CA.ChAtom S] -> [CA.ChAtom S] -> [CA.ChAtom S] -> [(CA.ChAtom S, E.Sigma, E.Gamma)] -> [CA.ChAtom S]
abstractChildIn ancs seen nodes []     =  nodes
abstractChildIn ancs seen nodes (b@(atom, subst, (x,y,delta)):bs) =
  case filter (\a -> CPD.embed a atom) ancs of
    -- add atom into nodes to add into the global tree
    [] -> trace ("\n" ++ show atom ++ "\n doesn't embed any of ancs:\n " ++ show ancs ++ "\n")
        $ abstractChildIn ancs (atom:seen) (b:nodes) bs
    -- If atom is a variant of any in seen then atom isn't added anywhere.
    _ | any (CPD.isVariant atom) seen -> trace ("\n" ++ show atom ++ "\n is variant of seen:\n " ++ show seen ++ "\n")
                                      $ abstractChildIn ancs seen nodes bs
    -- If atom isn't a variant and embeds al least one ancs then find msg and add it to bs.
    hs -> trace ("\nFind msg of\n (" ++ show atom ++ ")\n and\n {" ++ show hs ++ "}:\n")
          $ let (msgChatom, delta') = CA.msgChAtoms delta (atom:hs)
          in trace ("MSG: " ++ show msgChatom ++ "\n") $ let
          in abstractChildIn ancs seen nodes ((msgChatom, subst, (x, y, delta'))  : bs)
-}
      ...
      if lvl >= 5 then PruneCh (CPD.Descend goal ancs) gen sld
        ...
        let
         ...
         (unfoldNodes, otherNodes, newSeen) = foldl
           (\(unf, oth, see) gs ->
             let (var, new) = partition (\g -> CA.isSubst (absAtom g) || any (CPD.isVariant (absAtom g)) see) gs
             in
  ...
  go nodes goal ancs gamma subst generalizer
    = let
        ...
        (toUnfold, toNotUnfold, newNodes) = foldl
          (\(yes, no, seen) gs ->
            let (variants, brandNew) = partition (\(_, g, _, _) -> null g || any (CPD.isVariant g) seen) gs
...
abstract'
  ...
  -> E.Delta             -- Set of used semantic variables
  -> ([(CA.Atom S, T.Generalizer)], E.Delta)
...
abstractChild'
  ...
  -> [(E.Sigma, CA.Atom S, T.Generalizer, E.Gamma)]
